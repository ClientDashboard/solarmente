...
const additionalProjects = [
  ...
  { id: "5", title: "Casas - Costa del Este", capacity: "0 kW" }, // Cambiado a string en lugar de vacío
...
export default function ProyectosPage() {
  ...
  // Función para determinar la URL correcta según la extensión del archivo
  const getImageUrl = (id: string) => {
    if (id === "6") return `/images/${id}.webp`;
    if (parseInt(id) >= 41 && parseInt(id) <= 45) return `/images/${id}.png`;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {combinedProjects.map((project, index) => (
              !imageErrors[project.id] && (
                <div 
                  key={index}
                  className="bg-gray-900/40 rounded-xl border border-gray-800 overflow-hidden group transition-all duration-300 hover:border-solarmente-orange/50 hover:shadow-xl h-full flex flex-col cursor-pointer"
                  onClick={() => setSelectedImage(getImageUrl(project.id))}
                >
                  <div className="h-56 overflow-hidden relative">
                    ...
                    <div 
                      ...
                      style={{ backgroundImage: `url('${getImageUrl(project.id)}')` }}
